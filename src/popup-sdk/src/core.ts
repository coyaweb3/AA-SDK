...
import {
  ...
  AAuthMessage,
  UPConnectOptions,
  AATransactionMessage,
} from './types';
import { encodeTypedDataDigest, TypedData } from './utils';
...
import { getAuthProviderUrl, getDefaultConfigOption } from '.';
...
export class AAPopupSDK {
  ...
  private initConfig(options: PopupSDKOption) {
    ...
    this._config.chainType = options.chainType || 'ape';
  ...
  public updateConfig(options: Pick<PopupSDKOption, 'chainType' | 'nodeRPC'>) {
    if (!this._config) {
      return;
    }
   ...
   */
  public async login(options?: UPConnectOptions): Promise<AAccount> {
    return await connect(this._config!, options);
  }

  public getAccount(): AAccount | undefined {
  ...
  public async sendTransaction(
    _transaction: AATransactionMessage
  ): Promise<string> {
    this.assertLogin();
