import { AAMessage, AAResponse } from './types';
import { Callbacks } from './pop';

const POP = 'AA_WALLET_POP';
...
export async function renderPop(
  ...
  if (popup == null || popup?.closed) {
    ...
    if (popup == null) {
      return await createBlockedTip(src, onResponse);
    }
  ...
  var timer = setInterval(function () {
    if (popup && popup.closed) {
      const event = {
        data: new AAMessage(
          'UP_RESPONSE',
          JSON.stringify(new AAResponse('DECLINE', 'user reject connection'))
        ),
      } as MessageEvent;
      onResponse(event, {
        close: () => {},
  ...
  return new Promise((resolve) => {
    up_button?.addEventListener(
      ...
      function () {
        ...
        resolve(renderPop(src, onResponse));
      },
      false
    );
  });
